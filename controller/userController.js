...
exports.getUserById = CatchAsync(async (req, res, next) => {
    ...
    if (!user) {
        next(new AppError(`User ID ${id} not found`, 404));// this will tranfer this to globle error handler || handel operational error 
    }
    res.status(200).json({
        status: "success",
        data: user,
...
exports.createUser = CatchAsync(async (req, res, next) => {
    const { username, password, email, role } = req.body.XYZ;
